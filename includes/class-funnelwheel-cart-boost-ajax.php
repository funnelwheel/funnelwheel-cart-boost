...
/**
 * FunnelWheel_Cart_Boost_Ajax class.
 * @var [type]
 */
class FunnelWheel_Cart_Boost_Ajax {
	/**
	 * Constructor.
	 */
	...
	public function get_cart_information() {
		...
		if ( isset( $filtered_rewards['current_rewards'] ) && count( $filtered_rewards['current_rewards'] ) ) {
			...
			$reward_string      = funnelwheel_cart_boost()->rewards->get_reward_string( $filtered_rewards['current_rewards'] );
		}
		...
		wp_send_json(
			[
				'$_GET'                             => $_GET,
				'get_active_reward()'               => funnelwheel_cart_boost()->rewards->get_active_reward(),
				'current_reward_ids'                => $current_reward_ids,
				'is_empty'                          => WC()->cart->is_empty(),
				'items'                             => $cart_items,
				'gift_items'                        => $cart_gift_items,
				// translators: %d: Number of items in the cart.
				'cart_title'                        => sprintf( __( 'Your Cart (%d)', 'funnelwheel-cart-boost' ), WC()->cart->get_cart_contents_count() ),
				...
				'suggested_products'                => funnelwheel_cart_boost()->rewards->get_suggested_products(),
	...
	public function update_cart_item() {
		...
		if ( ! is_numeric( $quantity ) || $quantity < 0 || ! $cart_key ) {
			wp_send_json( [ 'error' => __( 'Something went wrong', 'funnelwheel-cart-boost' ) ] );
		}
	...
	public function get_suggested_products() {
		wp_send_json( funnelwheel_cart_boost()->rewards->get_suggested_products() );
	}
	...
	public function get_rewards() {
		wp_send_json( funnelwheel_cart_boost()->rewards->get_rewards() );
	}
